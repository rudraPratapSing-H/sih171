...
For a reproducible source build, KavachWeb pins the versions declared by the
...
continue through KavachWeb's existing audited ZIM reader.
...
2. a prominent notice that the package contains a modified javascript-libzim /
   libzim / Xapian Wasm build and the exact KavachWeb modifications and date;
...
6. a documented determination about the license of the combined extension and
   any required source release or relicensing of KavachWeb itself.
...
- KavachWeb **33.0.0 and later**, including the repository and Chrome, Edge, and
...
- `make libzim_release` downloads a **prebuilt** `libzim_wasm-emscripten-9.8.1`
  ...
  The upstream release workflow uses this. KavachWeb must not, because the
...
KavachWeb builds the source path. CI must archive every downloaded tarball and
...
KavachWeb's own reader answers the question instead: `hasFullTextIndex()` on the
