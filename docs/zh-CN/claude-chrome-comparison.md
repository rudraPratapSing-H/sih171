# KavachWeb 与 Claude Chrome 扩展对比

本文比较了本地的 KavachWeb 代码库与 `../webbrain-claude`（一个去混淆后的 Claude Chrome 扩展代码树）。重点比较架构、模型可调用工具以及特定网站的适配器行为。
...
KavachWeb 有两种在技能 Markdown 代码块中声明的工具类：
...
| 网络请求 | `fetch_url` / `research_url`，带有 KavachWeb 特定的 API 变更规则和用于可变方法的 `/allow-api`。 | 未恢复通用请求工具。通过 `read_network_requests` 存在调试网络日志。 |
...
| GIF/视频工作流 | KavachWeb Chrome 中存在斜杠驱动的录制，但不是以模型可调用的工具形式。 | `gif_creator` 是模型可调用的，可以录制/导出浏览器自动化会话为 GIF。 |

## 特定网站的适配器

### KavachWeb

KavachWeb 拥有真实的站点适配器系统：

- 适配器文件位于 `src/chrome/src/agent/adapters.js` 和 `src/firefox/src/agent/adapters.js`。
- `getActiveAdapter(url)` 返回第一个匹配的适配器。
- 一次仅触发一个适配器。
- 适配器注释放入第一条用户消息中。
- 如果导航在对话中途移动到不同的匹配适配器，KavachWeb 会注入一条新的 `[Site context changed ...]` 用户消息。
...
然而，在被检查的 Claude 代码树中，未找到支持的适配器注册表、`getActiveAdapter` 等效功能、通用前导注入或站点特定指导注入路径。搜索 KavachWeb 风格的适配器标记只找到了设置标签/存储路径。
