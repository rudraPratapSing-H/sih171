...
/**
 * KavachWeb Background Script (Firefox)
...
Promise.all([
  ...
  console.warn('[KavachWeb] Apocalypse Mode schedules could not be restored:', error);
});
    ...
    try {
      ...
      Promise.resolve(result).catch((e) => {
        if (!/duplicate/i.test(String(e?.message || e))) {
          console.warn('[KavachWeb] Failed to create context menu:', e?.message || e);
        }
      });
    } catch (e) {
      if (!/duplicate/i.test(String(e?.message || e))) {
...
// ────────────────────────────────────────────────────────────────────────
// Tab grouping (visual scope for a KavachWeb session)
//
...
// action, the source tab joins (or seeds) a colored "KavachWeb" tab group
 ...
 * "KavachWeb" group AND that `tab` is in it. Always creates a fresh group
...
// KavachWeb grouping as browser-action opens.
...
// in the KavachWeb group so the colored label appears immediately.
...
// KavachWeb" floating button. The chat / chat_stream / continue handlers
...
async function handleMessage(msg, sender) {
  ...
  switch (msg.action) {
    ...
    case 'set_help_improve_preference': {
      ...
      const stored = await browser.storage.local.get('helpImproveKavachWeb');
      const previousEnabled = stored.helpImproveKavachWeb !== false;
      await browser.storage.local.set({ helpImproveKavachWeb: msg.enabled });
      try {
        await providerManager.load();
      } catch (error) {
        if (previousEnabled !== msg.enabled) {
          await browser.storage.local.set({ helpImproveKavachWeb: previousEnabled }).catch(() => {});
        }
        throw error;
      }
...
browser.commands.onCommand.addListener(async (command, tab) => {
  ...
  try {
    ...
    console.error('[KavachWeb] failed to dispatch command:', command, err);
  }
});
